...
/// Resolves the Gemini API key for the IntraBadi assistant, mirroring
/// [OpenRouteServiceRouting]'s exact key-loading convention (see
...
class GeminiApiKeyLoader {
  GeminiApiKeyLoader({
    ...
    debugPrint(
      _compileTimeApiKey.isEmpty
          ? '[GeminiApiKeyLoader] No GEMINI_API_KEY compiled in via '
                '--dart-define — will attempt to load it from the bundled '
                'env.json asset on first use instead.'
          : '[GeminiApiKeyLoader] GEMINI_API_KEY compiled in via '
  ...
  Future<String> _loadKeyFromAssetFallback() async {
    try {
      ...
      debugPrint(
        key.isEmpty
            ? '[GeminiApiKeyLoader] env.json asset loaded but '
                  'GEMINI_API_KEY is empty/missing in it.'
            : '[GeminiApiKeyLoader] GEMINI_API_KEY loaded from the '
      ...
      debugPrint(
        '[GeminiApiKeyLoader] Could not load the env.json asset fallback '
        '(missing file, invalid JSON, or not declared as a bundled asset '
        'in pubspec.yaml): $e',
      );
      return '';
    }
