...
- **Base map**: `google_maps_flutter` (Android only in this build). Requires a Google Maps API key wired via native Android config — see setup below. If the key is missing/invalid or the map otherwise fails to load, the screen automatically falls back to a non-map POI list (same photos/names/categories, still tappable) instead of crashing or showing a blank screen.
...
- **Walking routes**: real, street-following directions between any two POIs, powered by the [OpenRouteService](https://openrouteservice.org/) Directions API (`foot-walking` profile) via `lib/services/routing_service.dart`. Unchanged by the base-map swap above.

> Note: `navigation_screen.dart` (live GPS turn-by-turn guidance) and `itinerary_navigation_overview_screen.dart` (itinerary route overview) are separate features that still use `flutter_map`/OpenStreetMap tiles — they were intentionally left as-is and are not affected by the Google Maps setup below.
...
If the key is left as the placeholder or is otherwise invalid, the map area shows a "Map unavailable — check the Google Maps API key" fallback with the POI list still browsable underneath, rather than crashing.
