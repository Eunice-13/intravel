...
void main() {
  ...
  test('does not keep answering about a previously-discussed location once '
    ...
    expect(discountResult.replyText.toLowerCase(), contains('discount'));

    // Then: an itinerary how-to question, also naming no location.
    // Must not describe Fort Santiago either.
    final howToResult = engine.process('how do I add a stop to my itinerary?');
    expect(howToResult.replyText, isNot(contains('seat of Spanish')));
  });

  test('a genuine vague follow-up ("how much does it cost") still resolves '
  ...
  test('a discount question naming no location lists real discounted '
  ...
  test('a direct "create an itinerary" request surfaces a pending '
  ...
  test('a "create an itinerary called X" request uses the given name', () {
    final engine = ChatbotConversationEngine();
  ...
  test('a discount question naming no location returns a numbered list with '
  ...
  test('a category question (e.g. "show me museums") returns a numbered '
